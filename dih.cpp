...
int main(int argc, char *argv[]) {
    ...
    layout->addRow(outputDisplay);
    layout->addRow("Enter usb:", usbInput);
    layout->addRow(fileButton);  // Fixed: was using undefined runButton
    ...
    for (int i = 0; i < lines.size(); i++) {
        ...
        if (devIndex != -1) {  
            QString disk = line.mid(devIndex).split(QRegExp("[:\\s]"), Qt::SkipEmptyParts).first();
            usbDiskPlacement.append(disk);
    }
}
    QString result;
